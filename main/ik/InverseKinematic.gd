...
func reach_toward(coordinate : Vector2):
	...
	if not model or not origin or not terminus or not is_ready:
		return
...
func cycle(target_position : Vector2) -> float:
	for link in ik_chain:
		...
		var rotation_amount = alpha_vector.angle_to(goal_vector) * sign(model.scale.x)
		if not pin_origin:
			if link == origin:
